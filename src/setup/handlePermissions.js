...
async function handlePermissions(cronObject) {
    ...
    log(`setup/handlePermissions|ln-12:  ${checkIfHasPermissions}`)
    ...
    log(`setup/handlePermissions|ln-23: ${checkIfHasPrompted}`)
    if (!checkIfHasPrompted) {
        ...
        log(
            "setup/handlePermissions|ln-29: We just showed the prompt by calling `hasScreenCapturePermission`, so we'll wait a bit to allow the user time to respond. This delay is arbitrary and in a real-world scenario, you might want to use events or other signals.",
        )
        let hasCapturePermissions = hasScreenCapturePermission()
        log(`setup/handlePermissions|ln-31: ${hasCapturePermissions}`)
